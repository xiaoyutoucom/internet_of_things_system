...
{
    public partial class Monitorimport : DevExpress.XtraEditors.XtraUserControl
    {
        private ILog _log = LogManager.GetLogger("Monitorimport");
        public Monitorimport()
        {
            InitializeComponent();
        }
        ...
        private void btn_ck_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var saveFileName = "监测点信息填写说明.xls";
            ...
            try
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        {
                            rich_result.Text += @"第" + row + "行监测点类型编码列不能为空" + "\n";
                        }
                        if (dt.Rows[i][3].ToString() == "")
                        {
                            rich_result.Text += @"第" + row + "行区划类型编码列不能为空" + "\n";
                        }
                    }
                    if (rich_result.Text == "")
                    {
